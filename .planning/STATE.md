...
| 260630-jm8 | RUN-06 fail-closed supervisor-image pin preflight: rebuild.sh verifies supervisor_image under [openshell.drivers.podman] is pinned to a non-:latest tag before sandbox create (read-only, version-agnostic). Defends against the floating supervisor:latest (pull policy "newer") drifting past the pinned gateway 0.0.62 → in-sandbox netns setup fails "Invalid argument (os error 22)" → "sandbox is not ready" | 2026-06-30 | 6a77d60 | [260630-jm8-supervisor-image-pin-preflight](./quick/260630-jm8-supervisor-image-pin-preflight/) |
| 260730-v3j | Sandbox-side opt-in support for locally hosted llama.cpp: commented-out claude-scoped local_model_egress template in policy.yaml (NET-06), claude-local launch verb + --base-url validation in rebuild.sh (RUN-07), fail-closed assert_local_model_egress_if_present() conditional assertion, tests/test-local-model-guard.sh negative-path guard test, and docs/local-models-guidelines.md host-side proxy build guide (D-01–D-04). Default two-allowlist posture unchanged; host-side proxy code explicitly out of scope (D-03) | 2026-07-30 | 7693ce7 | [260730-v3j-add-support-for-locally-hosted-llama-cpp](./quick/260730-v3j-add-support-for-locally-hosted-llama-cpp/) |
