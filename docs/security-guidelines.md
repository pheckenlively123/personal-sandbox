...
6. **The one sanctioned, documented exception to rule 2 is the opt-in `local_model_egress` block.** It is the single way to add a host to this policy without violating "never widen an allowlist": it ships **commented-out** by default (the active posture is unaffected until an operator deliberately uncomments it), it is `claude`-binary-scoped only with no `protocol` field (same passthrough rule as every other endpoint), its host is a placeholder (`REPLACE-ME-LOCAL-MODEL-HOST`) that is never guessed, and `NET-06` (`assert_local_model_egress_if_present` in `rebuild.sh`) asserts all of that fail-closed the moment it is enabled. See [`docs/local-models-guidelines.md`](local-models-guidelines.md) for the full opt-in path and its trade-offs. Do not treat this as license to add other hosts elsewhere — it is a single, narrowly-scoped, default-off exception, not a precedent for widening `claude_egress` or `go_egress`.
...
- [ ] If `local_model_egress` was touched: still commented-out by default, still `claude`-scoped only, still no `protocol` field, host still a placeholder (never a guessed real address) unless an operator is deliberately enabling it — and `NET-06` still asserts all of the above fail-closed.
